...
import { config, getRecommendedPairs, getAlternativePairs } from '../utils/config.js';
...
export class WebServer {
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/api/portfolio', (_req: Request, res: Response) => {
      ...
      const pairsArray = Array.from(state.pairs.entries()).map(([symbol, pairState]) => ({
        symbol,
        ...pairState,
    ...
    this.app.get('/api/trades/stats', (req: Request, res: Response) => {
      try {
        ...
        if (this.portfolioBot) {
          ...
          res.json({ totalTrades: 0, winningTrades: 0, losingTrades: 0, totalPnl: 0, avgPnl: 0, winRate: 0 });
        }
      } catch (error) {
  ...
  private setupWebSocket(): void {
    this.wss.on('connection', (ws: WebSocket) => {
      ...
      // Send initial status
      if (this.isPortfolioMode && this.portfolioBot) {
        ws.send(JSON.stringify({ type: 'portfolio', data: this.portfolioBot.getStatus() }));
      } else if (this.bot) {
  ...
  private startStatusBroadcast(): void {
    ...
    this.statusInterval = setInterval(() => {
      if (this.isPortfolioMode && this.portfolioBot) {
        this.broadcast({ type: 'portfolio', data: this.portfolioBot.getStatus() });

...
const ConfigSchema = z.object({
  ...
  gridLower: z.number().default(0.10),      // ~29% below current DOGE price
  ...
  riskStrategy: z.enum(['conservative', 'moderate', 'aggressive']).default('moderate'),
...
function loadConfig(): Config {
  const rawConfig = {
    ...
    gridUpper: process.env.GRID_UPPER ? parseFloat(process.env.GRID_UPPER) : undefined,
    gridLower: process.env.GRID_LOWER ? parseFloat(process.env.GRID_LOWER) : undefined,
    gridCount: process.env.GRID_COUNT ? parseInt(process.env.GRID_COUNT, 10) : undefined,
    amountPerGrid: process.env.GRID_AMOUNT ? parseFloat(process.env.GRID_AMOUNT) : undefined,
    ...
    totalCapital: process.env.TOTAL_CAPITAL ? parseFloat(process.env.TOTAL_CAPITAL) : undefined,
    riskStrategy: process.env.RISK_STRATEGY as 'conservative' | 'moderate' | 'aggressive' | undefined,
